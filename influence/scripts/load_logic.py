...
def init_lists(folder):
    ...
    for a_file in file_list:
        ...
        #file_list.append((f.read().decode("latin-1"),a_file))
    f.close()
    return a_list
#,file_list
...
def process_spam(n = None):
    ...
    spam = init_lists('data/logic/calendar/')
    ham = init_lists('data/logic/publications/')

    if n is None:
        docs, Y , nl_to_raw = nlprocessor.process_logic(spam, ham)
    else:
        docs, Y,nl_to_raw = nlprocessor.process_logic(spam[:n], ham[:n])
    num_examples = len(Y)
    ...
    for i in range(len(Y_train)):
        oneHot2Raw[i] = nl_to_raw[docs_train[i]] 
    return X_train, Y_train, X_valid, Y_valid, X_test, Y_test,oneHot2Raw


def load_logic(n = None):

    X_train, Y_train, X_valid, Y_valid, X_test, Y_test , oneHot2Raw = process_spam(n)
    ...
    return base.Datasets(train=train, validation=validation, test=test) , oneHot2Raw
