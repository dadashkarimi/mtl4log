...
class NeuralModel2(object):
  ...
  def __init__(self, spec, distract_num=0, float_type=np.float64):
    ...
    self.domain_size = spec.domain_size
  ...
  def sgd_step(self, ex, eta, l2_reg, distractors=None):
    ...
    """
    #print 'x: %s' % ex.x_str #FIXME
    #print 'y: %s' % ex.y_str
    if distractors:
      #for ex_d in distractors:
      #  print 'd: %s' % ex_d.x_str FIXME
      x_inds_d_all = [ex_d.x_inds for ex_d in distractors]
      info = self._backprop_distract(
          ex.x_inds, ex.y_inds, eta, ex.y_in_x_inds,ex.y_in_src_inds,ex.y_in_domain, l2_reg, *x_inds_d_all)

    else:
      info = self._backprop(ex.x_inds, ex.y_inds, eta, ex.y_in_x_inds,ex.y_in_src_inds,ex.y_in_domain,  l2_reg) # for attn2hist
  ...
  def train(self, dataset, eta=0.1, T=[], verbose=False, dev_data=None,
    ...
    for it in range(max_iters):
      ...
      if augmenter:
        ...
        aug_exs = [Example(
            x, y, z, dataset[0].input_vocab, dataset[0].output_vocab, 
            dataset[0].lex, reverse_input=dataset[0].reverse_input)
            for x, y, z in augmenter.sample(aug_num)]
      ...
      if concat_num > 1:
        ...
        for i in range(num_concat_exs / concat_num):
          ...
          new_z_str = (' ' + Vocabulary.END_OF_SENTENCE + ' ').join(
              ex.z_str for ex in cur_exs)
          new_ex = Example(new_x_str, new_y_str, new_z_str, dataset[0].input_vocab,
      ...
      if dev_data:
        for ex in dev_data:
          dev_nll += self._get_nll(ex.x_inds, ex.y_inds, ex.y_in_x_inds,ex.y_in_src_inds,ex.y_in_domain)
