...
class NeuralModel(object):
  ...
  """
  def __init__(self, specs, distract_num=0, float_type=numpy.float64):
    """Initialize.
    ...
    """
    self.specs = specs
    self.spec = specs['_shared_']
    self.lexicon = self.spec.lexicon
    self.distract_num=distract_num
    self.float_type = float_type
    params = []
    for domain in specs:
      params += specs[domain].get_params()
    self.params = params
    if self.spec.step_rule in ('adagrad', 'rmsprop', 'nesterov'):
    ...
    all_shared = []
    for domain in specs:
      all_shared += specs[domain].get_all_shared()
  ...
  def sgd_step(self, ex, eta, l2_reg, distractors=None):
    ...
    if distractors:
      ...
      info = self._backprops[ex.sub_domain](ex.x_inds, ex.y_inds, ex.d_inds, eta, ex.y_in_x_inds, l2_reg)
    p_y_seq = info[0]
    objective = info[1]
    ...
    for it in range(max_iters):
      ...
      if dev_data:
        for ex in dev_data:
          ...
          dev_nll += self._get_nlls[domain](ex_shared.x_inds, ex_domain.x_inds, ex_shared.d_inds, ex_domain.y_inds, y_in_x_inds)
