...
class Vocabulary:
  ...
  def __init__(self, word_list, emb_size, use_glove=False, special_tokens=True, fixed_embed=False, float_type=numpy.float64,
               unk_cutoff=0):
    ...
    self.fixed_embed = fixed_embed
    self.float_type = float_type

    # Embedding matrix
    if self.fixed_embed:
      init_val = 0.1 * numpy.identity(self.size()).astype(theano.config.floatX)
    else:
  ...
  @classmethod
  def from_sentences(cls, sentences, emb_size, unk_cutoff=0, special_tokens=True, fixed_embed=False, **kwargs):
    ...
    return cls(word_list, emb_size, special_tokens=special_tokens, fixed_embed=fixed_embed, **kwargs)
