...
class AttentionSpec(Spec):
  ...
  def create_vars(self):
    ...
    self.bwd_encoder = self.create_rnn_layer(
        self.hidden_size, self.in_vocabulary.emb_size,
        self.in_vocabulary.size(), True)
    if self.domain != '_shared_':
      self.w_enc_to_dec = theano.shared(
        name='w_enc_to_dec',
        # account for double encoder size (domain + shared)
        value=0.1 * numpy.random.uniform(-1.0, 1.0, (dec_full_size, 2 * annotation_size)).astype(theano.config.floatX))

      self.decoder = self.create_rnn_layer(
          self.hidden_size, self.out_vocabulary.emb_size + 2 * annotation_size + self.domain_vocabulary.emb_size,
          self.out_vocabulary.size(), False)
      # there are annotations over domain and shared encoder
      self.writer = self.create_output_layer(self.out_vocabulary,
                                             self.hidden_size + 2 * annotation_size)
      self.w_attention = theano.shared(
        name='w_attention',
        #account for double encoder size (domain + shared)
        value=0.1 * numpy.random.uniform(-1.0, 1.0, (self.hidden_size, 2 * annotation_size)).astype(theano.config.floatX))


  def get_local_params(self):
    if self.domain == '_shared_':
      params = self.fwd_encoder.params + self.bwd_encoder.params
    else:
      params = (self.fwd_encoder.params + self.bwd_encoder.params +
  ...
  def f_dec(self, y_t, d_t, c_prev, h_prev):
    ...
    if self.domain_vocabulary.emb_size > 0:
      d_emb_t = self.domain_vocabulary.get_theano_embedding(d_t)
