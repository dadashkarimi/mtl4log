...
class AttentionSpec(Spec):
  ...
  def create_vars(self):
    ...
    if self.domain == '_shared_':
      ...
      self.decoder = self.create_rnn_layer(
          self.hidden_size, self.out_vocabulary.emb_size + annotation_size + self.domain_vocabulary.emb_size,
  ...
  def get_local_params(self):
    if self.domain == '_shared_':
      params = self.fwd_encoder.params + self.bwd_encoder.params + [self.w_enc_to_dec]
    else:
      params = self.decoder.params + self.writer.params
    return params
