...
from attnspec_multi import AttentionSpecMulti
...
class AttentionModelMulti(NeuralModel):
  ...
  def setup_backprop(self, domain):
    ...
    # Normal operation
    shared_enc, annotations = self._symb_encoder_domain(x, '_shared_')
    #dec_init_state_domain, annotations_domain = self._symb_encoder_domain(x_domain, domain)

    nll, p_y_seq, objective, updates  = self._setup_backprop_with(
      shared_enc, annotations, y, d, y_in_x_inds, eta, l2_reg)
    self._get_nlls[self.domain] = theano.function(
        inputs=[x, y, d, y_in_x_inds], outputs=nll, on_unused_input='warn')
        #inputs=[x, x_domain, y, y_in_x_inds], outputs=nll, on_unused_input='warn')
    self._backprops[self.domain] = theano.function(
        inputs=[x, y, d, eta, y_in_x_inds, l2_reg],
        #inputs=[x, x_domain, y, eta, y_in_x_inds, l2_reg],
    ...
    if self.distract_num > 0:
      ...
      nll_d, p_y_seq_d, objective_d, updates_d = self._setup_backprop_with(
        shared_enc, annotations_with_distract, shared_enc, y, y_in_x_inds, eta, l2_reg)
  ...
  def _setup_backprop_with(self, dec_init_state, annotations_domain, y, d, y_in_x_inds,
                           eta, l2_reg):
    ...
    dec_results, _ = theano.scan(
        ...
        non_sequences=[annotations_domain] + self.all_shared)
  ...
  def decode_beam(self, ex, beam_size=1, max_len=100):
    ...
    domain = ex_domain.sub_domain
    #h_t, annotations = self._encode_domain[domain](ex_domain.x_inds)
    #shared_enc, _ = self._encode_domain['_shared_'](ex_shared.x_inds)
    h_t, annotations = self._encode_domain['_shared_'](ex_shared.x_inds)
  ...
  def sgd_step(self, ex, eta, l2_reg, distractors=None):
    ...
    if distractors:
      ...
      info = self._backprops[domain](ex_shared.x_inds, ex_domain.y_inds, ex_shared.d_inds, eta, y_in_x_inds, l2_reg)
    p_y_seq = info[0]
    objective = info[1]
