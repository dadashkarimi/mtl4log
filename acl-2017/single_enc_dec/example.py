...
class Example(object):
  ...
  """
  def __init__(self, x_str, y_str, input_vocab, output_vocab, domain_vocab, lex,
               reverse_input=False, sub_domain=None):
    ...
    domain_ind = domain_vocab.sentence_to_indices(sub_domain, add_eos=False)
    domain_inds = []
    for i in range(len(self.y_inds)):
      domain_inds.append(domain_ind[0])
    self.d_inds = domain_inds
