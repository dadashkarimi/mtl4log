...
def _parse_args():
  ...
  parser.add_argument('--domain-embedding-dim', '-e', type=int, default=0,
                      help='Dimenstion of domain embedding vecore. If size is zero domain embeddings are not used.'
                           ' If -1 domain vectors are one-hot vectors indicating the domain')
  parser.add_argument('--input-domain-token', action='store_true',
                      help='Addition of an extra artificial token at the beginning of the input sentence which specifies the domain.')
  parser.add_argument('--masking-dict', help='Path to a dict containing relation masking.')
  ...
  parser.add_argument('--domain', default='overnight-calendar',
...
def preprocess_data(model, raw):
  ...
  for raw_ex in raw:
    ...
    ex = Example(x_str, y_str, in_vocabulary, out_vocabulary, domain_vocabulary, lexicon,
                 reverse_input=OPTIONS.reverse_input, sub_domain=sub_domain)
    data.append(ex)
  return data

def get_spec(in_vocabulary, out_vocabulary, domain_vocabulary, out_vocabularies, lexicon):
  ...
  return constructor(in_vocabulary, out_vocabulary, domain_vocabulary, out_vocabularies, lexicon,
...
def evaluate(name, model, dataset, domain=None):
  ...
  if domain:
    ...
    # unmask logical form to have their original peroerty names
    if OPTIONS.masking_dict:
      masking = pickle.load(open(OPTIONS.masking_dict, 'rb'))
      for sample in all_derivs:
        for deriv in sample:
          unmask_tokens(deriv.y_toks, masking)
      true_answers_unmasked = []
      for sample in true_answers:
        toks = sample.split()
        unmask_tokens(toks, masking)
        true_answers_unmasked.append(' '.join(toks))
      true_answers = true_answers_unmasked
...
def unmask_tokens(tokens, masking):
  for i, y_tok in enumerate(tokens):
    if y_tok in masking:
      print('replaced {} {}'.format(y_tok, masking[y_tok]))
      tokens[i] = masking[y_tok]
...
def load_raw_all(domain=None):
  ...
  if OPTIONS.train_data:
    ...
    if OPTIONS.dev_frac > 0.0 and not OPTIONS.dev_data:
...
def init_spec(train_raw):
  if OPTIONS.load_file:
    ...
    lexicon = get_lexicon()
    spec = get_spec(in_vocabulary, out_vocabulary, domain_vocabulary, out_vocabularies, lexicon)
  else:
