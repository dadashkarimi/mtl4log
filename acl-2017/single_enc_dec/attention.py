...
class AttentionModel(NeuralModel):
  ...
  def setup_decoder_step(self):
    ...
    self._decoder_step = theano.function(inputs=[y_t, d_t, c_prev, h_prev], outputs=h_t, on_unused_input='ignore')
  ...
  def setup_decoder_write(self):
    ...
    write_dist = self.spec.f_write(h_for_write, c_t, scores, mask)
    self._decoder_write = theano.function(
        inputs=[annotations, h_prev, mask], outputs=[write_dist, c_t, alpha])
  ...
  def decode_beam(self, ex, beam_size=1, max_len=100):
    mask = numpy.asarray(
      [1 if self.out_vocabulary.get_word(y) in self.out_vocabularies[ex.sub_domain].word_list else 0 for y in
       range(self.out_vocabulary.size())])
    #mask = numpy.ones(self.out_vocabulary.size())
    h_t, annotations = self._encode(ex.x_inds)
    ...
    for i in range(1, max_len):
      ...
      for deriv in beam[i-1]:
        ...
        write_dist, c_t, alpha = self._decoder_write(annotations, h_t, mask)
