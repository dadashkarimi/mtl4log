...
class Example(object):
  ...
  """
  def __init__(self, x_str, y_str, z_str, input_vocab, output_vocab, domain_size, lex,
               reverse_input=False):
    ...
    self.y_in_domain = ([[0 for i in range(self.domain_size)] for y_tok in self.y_toks]+[[0]*(self.domain_size)])
 
    for i in range(len(self.y_in_domain)):
        for j in range(len(self.y_in_domain[0])):
            if (j==int(z_str)):
                self.y_in_domain[i][j] = 1
